...
inline void Buffer::writeToFile(const std::string& filename) const {
  std::ofstream file(filename);
  const std::size_t s = size();
  file.write(reinterpret_cast<const char*>(&s), sizeof(std::size_t));
  file.write(reinterpret_cast<const char*>(data()), s);
}

inline void Buffer::readFromFile(const std::string& filename) {
