...
We \replaced{to implement}{tried implementing} the parallel execution of loop \ref{algorithm:step:loop} with  Boost fibers
\cite{Boost}
whose execution is scheduled with a work stealing algorithm, and OpenMP with a dynamic scheduler.
OpenMP performed better by a large margin and \replaced{and thus was}{will be} used to acquire the data presented later.
...
with a known geographical position\deleted{, posses this property}.
...
one sided communication. The list \replaced{local vertex ids}{of ids of the local vertices} is stored in an MPI window, so that
