...
Daint setups. We will then go on to discussing each experiment \added[id=R]{seperately}.
...
and \added[id=R]{still missing, just so we dont forgett}

\mypar{Graph Generation}
Our algorithm was evaluated on undirected, unweighted graphs. Multiple edges connecting the same
two vertices and self loops were not allowed. All graphs were generated using \cite{Parmat} \added[id=R]{to generate the same type of graphs as used in \cite{comm_avoiding}.}.
...
\autoref{fig:mpi_omp_commavoiding_euler_1} shows the MPI \deleted[id=R,remark=I deleted all the onlys. I think it hurts the flow.]{only} version outperforming the OMP version on the densest graph. This can be explained by the combination of two effects\replaced[id=R,remark=long sentence split in two]{. The}{: the}:
first \replaced[id=R]{is}{being} that a dense graph results in more contention between the OMP threads during the
edge contractions, and the second \replaced[id=R]{is}{being} that the reduction \replaced[id=R]{afterwards}{after the contractions} scales linearly
with the number of vertices. Since the number of vertices is comparatively low in a dense graph the
reduction is fast. For the OMP \deleted[id=R]{only} version we further observe a significant increase in total
...
\autoref{fig:mpi_omp_commavoiding_euler_1}. Here, for a large number of cores, the OMP \deleted[id=R]{only} version
is clearly faster than the MPI \deleted[id=R]{only} version. \autoref{fig:mpi_omp_commavoiding_euler} shows a trend
of the OMP \deleted[id=R]{only} version speeding up as the graph becomes sparser, while the MPI \deleted[id=R]{only} version slows
down. The OMP \deleted[id=R]{only} version's speed up can be explained by \replaced[id=R]{less}{the reduced} contention between the OMP
threads due to the sparser graph. The MPI \deleted[id=R]{only} version's slow down is a result of the \replaced[id=R]{longer}{increased}
reduction time due to the \replaced[id=R]{larger}{increasing} number of vertices.\\
...
on a single node \deleted[id=R]{in this algorithm}. Since our algorithm does to scale with the number of cores up
...
\replaced[id=R]{Aditionally we tested our algorithm on the Piz Daint cluster.} {In addition to testing our algorithm on the Euler cluster it was also tested on the Piz Daint cluster}. The graphs used were the same as in the experiments run on Euler. We will now discuss interesting differences in the results.\\
\autoref{fig:mpi_omp_commavoiding_daint_1} shows the results for the densest graph on Piz Daint. The OMP \deleted[id=R]{only} version behaves differently compared to Euler. We still observe a decrease in performance when going from one to two cores. The decrease is not as severe as on Euler. The performance decrease as we go from one to multiple CPUs is no longer present. This can be explained by Daint's cache coherency protocol being more efficient, especially across multiple CPUs.
\autoref{fig:mixed_daint} shows the Piz Daint results for different mixtures of MPI and OMP on the sparsest graph. As on Euler, we do see the OMP \deleted[id=R]{only} version performing best for a large number of cores. For all versions, except the OMP \deleted[id=R]{only} version, we can clearly see the effect the number of reduction steps has on the runtime. It is most noticeable as we go from 16 to 18 cores. Here each version has to perform an additional reduction step which results in the increased runtime.
...
\autoref{fig:speedup} shows the measured speed up of the MPI \deleted[id=R]{only} and OMP \deleted[id=R]{only} version. As one
would expect from the results discussed previously the MPI \deleted[id=R]{only} version achieves better scaling on dense graphs while the OMP \deleted[id=R]{only} version scales better on sparse graphs.\\
We can also see that our algorithm achieves better scaling on Euler for the MPI \deleted[id=R]{only} version. The reason our algorithm scales better on Euler is that the single core performance is lower on Euler compared to Daint. This means that the edge contractions dominate the runtime for longer before the reduction step starts to be an issue.\\
For the OMP \deleted[id=R]{only} version the \replaced[id=R]{multiple}{differences} in speed ups can be explained by the differences in the shared memory architecture.
