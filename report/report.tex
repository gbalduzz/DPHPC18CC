...
introduced by \cite{PCompArticle}. To improve the algorithm's performance the graph's edges are % NOTE: no citations in abstract
distributed across multiple cores. Each core computes in a lock-free manner a local forest representing the different
connected components. The resulting forests are then combined using a reduction. A comparison of our
algorithm against a communication avoiding algorithm \cite{comm_avoiding} on a set of large graphs
