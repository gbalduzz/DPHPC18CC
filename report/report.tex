...
introduced by \replaced{U. Vishkin}{\cite{PCompArticle}}. To improve the algorithm's performance the graph's edges are % NOTE: no citations in abstract
distributed across multiple cores. Each \replaced{core}{process} computes in a lock-free manner a local forest representing the different
connected components. The resulting forests are then combined using a reduction. A comparison of our
algorithm against a communication avoiding algorithm \replaced{proposed by Lukas Gianinazzi et al.}{\cite{comm_avoiding}} on a set of large graphs
