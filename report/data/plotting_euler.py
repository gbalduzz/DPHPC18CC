...
nr_vertices = [10000000,500000,20000000]
...
def plot_mean_and_CI(x, mean, lb, ub, color_mean, color_shading, linewidth=1, label=None):
    ...
    plt.plot(x, mean, color_mean, linewidth=linewidth, label=label,marker='o', markersize=5)
...
for j,benchmark_dir in enumerate([benchmark_dir1, benchmark_dir2, benchmark_dir3]):
	...
	for i in range(0, 24): 
	    ...
	    u += [up]
	    l += [lw]
	plot_mean_and_CI([i for i in range(1,25)], to_plot, u, l,color_mean="red", linewidth=linewidth, color_shading="#ff9372", label="omp only")
	...
	plt.ylabel("Time [s]")
	plt.title('500M edges, ' + format(nr_vertices[j]) + ' vertices.')
	plt.ylim(bottom=0, top=45)
	plt.legend()
	plt.tight_layout(pad=0.1,w_pad=0.1,h_pad=0.1)
	
	plt.savefig(benchmark_dir[0:5] + "our_impl.pdf", format='pdf')
