...
def plot_mean_and_CI(x, mean, lb, ub, color_mean, color_shading, linewidth=1, markerSize=1,  label=None):
	...
	plt.fill_between(x, ub, lb, alpha=.5, color=color_shading)
	# plot the mean on top
	plt.plot(x, mean, linewidth=linewidth, marker='o', markersize=markerSize, label=label, color=color_mean)
...
for benchmark_count, benchmark_dir in enumerate([benchmark_dir1, benchmark_dir2, benchmark_dir3]):
	if(doComparison):
		# setup plots
		...
		if(benchmark_count !=0): 
			...
			for i, name in enumerate(["Scatter", "Compute", "Reduce", "Total compute"]):
				...
				plt.sca(axarr[i])
				plot_mean_and_CI(ns, data[:,0], data[:,1], data[:,2], color_mean='orange', color_shading="#f2f237", linewidth=linewidth, markerSize=markerSize, label="mixed (" + str(t) + " threads per node)")
			...
			for i, name in enumerate(["Scatter", "Compute", "Reduce", "Total compute"]):
				data = np.array([confidenceInterval(getSamples(name, getMixedFilename(n, t), int(n/t)), confidence_value) for n in ns])
				plt.sca(axarr[i])
				plot_mean_and_CI(ns, data[:,0], data[:,1], data[:,2], color_mean='magenta', color_shading="#FCA387", linewidth=linewidth, markerSize=markerSize, label="mixed (" + str(t) + " threads per node)")
			t = 8
			#For 20
			if(benchmark_count == 2):
			...
			for i, name in enumerate(["Scatter", "Compute", "Reduce", "Total compute"]):
				data = np.array([confidenceInterval(getSamples(name, getMixedFilename(n, t), int(n/t)), confidence_value) for n in ns])
				plt.sca(axarr[i])
				plot_mean_and_CI(ns, data[:,0], data[:,1], data[:,2], color_mean='black', color_shading="#778191", linewidth=linewidth, markerSize=markerSize, label="mixed (" + str(t) + " threads per node)")
		...
		for i in range(0, 24): 
			...
			u += [up]
			l += [lw]
		plot_mean_and_CI([i for i in range(1,25)], to_plot, u, l,color_mean="red", linewidth=linewidth, markerSize=markerSize, color_shading="#ff9372", label="OMP")
		...
		plt.sca(axarr[3])
		plot_mean_and_CI(ns, data[:,0], data[:,1], data[:,2], color_mean='green', color_shading="#87D37C", linewidth=linewidth, markerSize=markerSize, label="comm avoiding")
		...
		for i, ax in enumerate(axarr):
			...
			plt.ylabel("Time [s]")
			plt.ylim(bottom=0, top=axis_tops[i])
		...
		plt.savefig(benchmark_dir[0:5] + "mpi_mixtures_with_everything.eps", dpi=40, format='eps')
